  ...
  clipAndSaveRecipe(recipe, waitForElement=false) {
    try {
      ...
      if (!node) {
        if (waitForElement) {
          // We'll wait for 10s
          return _tcWaitForElement(recipe.selector, 10 * 1000)
              .then(() => this.clipAndSaveRecipe(recipe, false));
        }
      ...
      const recipeWrapper = new RecipeTableWrapper(recipe, node, pageUrl, pageTitle, windowName, this.userConfig_);
      const { dataArray } = recipeWrapper.getData();
