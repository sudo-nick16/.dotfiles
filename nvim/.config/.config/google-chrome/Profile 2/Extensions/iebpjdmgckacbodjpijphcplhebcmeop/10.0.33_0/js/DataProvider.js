...
class DataProvider {
  ...
  isPdfPage() {
    return chrome.extension.getBackgroundPage().isActiveTabPdfPage();
  }

  isXmlPage() {
    return chrome.extension.getBackgroundPage().isActiveTabXmlPage();
  }

  isGoogleSheetsAppPage() {
    const url = this.getCurrentWindowLocation();
    return url && url.includes('docs.google.com/spreadsheets') && !url.includes('htmlview');
  }

  isPowerBiPage() {
    const url = this.getCurrentWindowLocation();
    return url && (url.includes('https://powerbi.com/') || url.includes('https://app.powerbi.com/'));
  }

  isAirtableAppPage() {
    const url = this.getCurrentWindowLocation();
    return url && (url.includes('https://airtable.com/') || url.includes('https://www.airtable.com/'));
  }
  ...
  getSelectedElementDef() {
    ...
    const tableDefs = chrome.extension.getBackgroundPage().getActiveTableDefs();
    if (tableDefs) {
      Object
          .keys(tableDefs)
          .forEach(key => {
    ...
    if (tableDefs) {
      ...
      Object
          ...
          .forEach(key => {
            ...
            } else if (def.selectedTableWrapperIndex !== null && def.tables && def.selectedTableWrapperIndex < def.tables.length) {
  ...
  sendMessageToActiveTab(params) {
    ...
    this.activityLogger_.logEvent(`table-message-${action}`, {action});

    return new Promise((resolve, reject) => {
      this.getActiveTabId_()
        .then(tabId => {
  ...
  incrementSelectionAttemptCount () {
    ...
    this.browserEnv_
        .getSyncStorageApi()
        .setP({[_TCAP_CONFIG.selectionAttemptKey]: this.selectionAttemptCount_});
  }
