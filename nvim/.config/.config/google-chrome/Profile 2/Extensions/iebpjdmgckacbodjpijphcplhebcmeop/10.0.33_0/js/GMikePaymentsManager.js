...
class GMikePaymentsManager {
  ...
  bindToCancelLink() {
    const cancelUrl = `${this.manageLicenseUrl_}${this.licenseCode_}`;
    document.querySelector('a.subscriptions').href = cancelUrl;
  }
  ...
  isLicenseProductAcceptable_({product}) {
    return product === this.licenseProduct_
        || this.adjacentProducts_.includes(product);
  }
