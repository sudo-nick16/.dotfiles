...
window.setTimeout(sendPairsAl, 4000);
  }
  else
  {
          console.log("");
...
function sendPairsAl() {
  $s = jQuery.noConflict();
    var arrayToSend = [];
    var slider = $s('.product-view');
...
var deci1;
    ...
    if (PID) {
        ...
        cPID=PID;
        currentPrice=$s("div.product-view .Price-current .Price-int font font:eq(0)").text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        deci=$s("div.product-view .Price-current .Price-dec font font:eq(0)").text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        if (currentPrice == "" || currentPrice == undefined)
        {
             currentPrice=$s("div.product-view.product-page-pricing .product-old-price s font:eq(0)").text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
             deci=$s("div.product-view.product-page-pricing .product-old-price s sup font:eq(0)").text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        }
            if (currentPrice == "" || currentPrice == undefined)
        {
             currentPrice=$s("div.product-view.product-page-pricing .product-new-price").clone()    //clone the element
    .children() //select all the children
    .remove()   //remove all the children
    .end().text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
     deci=$s("div.product-view.product-page-pricing .product-new-price sup").text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        ...
        cPrice=currentPrice;
        title=$s("h1").text().replace(' ', '').replace('\n', '');
       cTitle=title;
        imgurl = $s("div.product-view .gallery-container img").attr('src');
 ...
 if(ptban.split('ptbanpid=')[1])
  current_status = 5;
    ...
    for(i=0; i<$s('li.Products-item').length; i++)
        {
            try{
            PID = returnPID($s('li.Products-item:eq('+i+') a').attr('href'));
            title=$s('li.Products-item:eq('+i+') a').attr('title');
              imgurl=$s('li.Products-item:eq('+i+') a').find('img').attr('src');

            //price =$s('li.Products-item:eq('+i+') p.product-new-price').text().replace(/[^\d.]/g, '').replace(/^\./, '').replace(",", "").replace(",", "");
                price=$s('li.Products-item:eq('+i+') .Price-current .Price-int font font:eq(0)').text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        deci1=$s('li.Products-item:eq('+i+') .Price-current .Price-dec font font:eq(1)').text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        if (price == "" || price == undefined)
        {
             price=$s('li.Products-item:eq('+i+') p.product-old-price s font:eq(0)').text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
             deci1=$s('li.Products-item:eq('+i+') p.product-old-price s sup font:eq(0)').text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
        }
            if (price == "" || price == undefined)
        {
             price=$s('li.Products-item:eq('+i+') p.product-new-price').clone()    //clone the element
    .children() //select all the children
    .remove()   //remove all the children
    .end().text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
     deci1=$s('li.Products-item:eq('+i+') p.product-new-price sup').text().replace(' ', '').replace('\n', '').replace(",", "").replace(",", "");
    ...
    if (arrayToSend.length == 0){ if(iii) return 0; else iii=1; return window.onload =  sendPairsAl;}
  $s("#containerMainPTID").remove();
    arrayToSend = JSON.stringify(arrayToSend);
    console.log("Altex Final Price:"+arrayToSend);
...
if(pwidth>816)
{
  ...
  else
  {
    ...
    selector.push({selector: 'div.Product:eq(0)', attr: 'none', pos: 'after'});
   ...
   var  impposcssdata="";
    ...
    var passBack1 = [{title: prod, siteName: 'Altex', price: pr, store:'altex',currency:'Lei ',ccode:'Lei '}];
...
pwidth=pwidth-512;
    ...
    selector.push({selector: '.mprod-section:eq(0)', attr: 'none', pos: 'after'});
    selector.push({selector: '._2fCBwf._3S6yHr:eq(0)', attr: 'parent', pos: 'after'});*/
    selector.push({selector: '.Price.Price--discount:eq(0)', attr: 'none', pos: 'after'});
    selector = JSON.stringify(selector);
    height = "1050px";
   var  impposcssdata="";
    var passBack = [{selectors: selector, height: height}];
    passBack = JSON.stringify(passBack);
        var passBack1 = [{title: prod, siteName: 'Altex', price: pr, store:'altex',pid:prodid,color:'#2874f0'}];
        passBack1 = JSON.stringify(passBack1);
...
function getAvailability(){
  ...
  else if($s('.product-view.product-page-pricing span').text().toUpperCase().split("SOLD OUT").length > 1){
    avail = 0;
  }
