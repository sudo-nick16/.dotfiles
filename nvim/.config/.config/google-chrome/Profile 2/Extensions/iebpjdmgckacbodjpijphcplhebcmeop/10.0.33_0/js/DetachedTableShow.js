...
class DetachedTableShow {
  ...
  display() {
    if (this.userConfig_.requiresPaid && !this.userConfig_.paidPro) {
      return alert('Please activate Table Capture first.');
    }
    ...
    if (validTableWrappers.length) {
      ...
      try {
        alert('There are no table elements on this page. Right-click to launch the Table Capture Workshop to capture your data.');
      } catch (err) {}
    }
  }
