...
class SheetManager {
  ...
  fetchAndRender_(postUpdate) {
    return this.sheetsSyncBridge_.getSheets()
  ...
  renderSheets_(sheetMap, postUpdate) {
    ...
    if (!sheets || sheets.length === 0) {
      const alert = this.createAlert_("You have no active Google Sheets.", "warning", false);
    ...
    sheets.forEach(sheet => {
      const sheetTitle = sheet.userTitle || sheet.title || sheet.pageTitle || '';
      const sheetEl = document.createElement('div');
      sheetEl.className = "sheet-list-item";
      sheetEl.innerHTML = `
        <div>
          <input type="text" class="form-control" value="${sheetTitle}" ${sheet.hidden ? "READONLY" : ""} />
          <div class="metadata">
      ...
      sheetEl.querySelector('.save-btn').addEventListener('click', () => {
        this.updateSheetEntry_({...sheet, userTitle: sheetEl.querySelector('input.form-control').value});
      });
