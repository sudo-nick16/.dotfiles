...
function handlePagingContinuation(req) {
  ...
  selectionWorkshop.setPagingData(data, count, continuationKey, treatAsTable, pathToElement, pathToPager, pagerSelector, autoPageTable);
...
function initTableMan() {
  ...
  getExtensionUserConfig()
    .then(userConfig => {
      if (userConfig && typeof _tcxLogger !== "undefined" && _tcxLogger) {
        _tcxLogger.setEnabled(userConfig.enableRequestSearch);
      }
