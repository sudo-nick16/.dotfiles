...
var cPID="";
      if ($s('div.v-pro-info')
        ...
        var PID=window.location.href;
            ...
            PID=PID+".html";
                cPID=PID;
            price = filter_price($s('span.v-offer-price-amt')
                .text()
                .replace('Rs.', '')
                .replace(',', ''));
            ...
            cPrice=price;
            title=$s('div.v-product-content h1:eq(0)').text();
            cTitle=title;
                imgurl=$s('div.vipImage__product').find('img')
                    .attr('src');
...
function plotFlipGraph(prodid,title,price){
  ...
  if($s("#containerMainPTID").length == 0){
    ...
    var pr=price;
    var selector = [];
    selector.push({selector: 'div.v-pro-info:eq(0)', attr: 'none', pos: 'after'});
...
pwidth=pwidth-512;
    ...
    selector.push({selector: '.mprod-section:eq(0)', attr: 'none', pos: 'after'});
    selector.push({selector: '._2fCBwf._3S6yHr:eq(0)', attr: 'parent', pos: 'after'});*/
    selector.push({selector: 'div.v-offer-price-wrap:eq(0)', attr: 'none', pos: 'after'});
    selector = JSON.stringify(selector);
    height = "1050px";
   var  impposcssdata="";
    var passBack = [{selectors: selector, height: height}];
