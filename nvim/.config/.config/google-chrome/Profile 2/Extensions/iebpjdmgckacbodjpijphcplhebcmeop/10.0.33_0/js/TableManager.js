...
class TableManager {
  ...
  initSelectedElement() {
    ...
    if (_tcIsNodeWithinTable(this.selectionNode_)) {
      this.selectedTableWrapper_ = this.findTableWrapperViaSelection(this.selectionNode_);
      return;
    }

    const url = window.location.href;
    this.selectionWrapper_ = new SelectionWrapper(this.selectionNode_, url, this.userConfig_);
    this.selectedTableWrapper_ = null;
  }
  ...
  findTableWrapperViaSelection(node) {
    ...
    const tableWrappers = this.tables_.filter(wrapper => wrapper.getDomElement() == tableNode);
  ...
  destroy () {
    this.getTables()
        .forEach(tableWrapper => tableWrapper.destroy());
    this.tables_ = [];
  ...
  findTables () {
    ...
    if (href && href.includes('google.com/spreadsheets') && !href.includes('htmlview')) {
      return;
    }
            ...
            if (href.match(new RegExp(recipe.urlRegex))) {
              ...
              if (element) {
                const recipeWrapper = new RecipeTableWrapper(
                    recipe, element, href, pageTitle, windowName, this.userConfig_);
    ...
    if (hostname.endsWith('.notion.so') || hostname.endsWith('.notion.site')) {
      Array
          .from(document.querySelectorAll('.notion-table-view'))
          .forEach(table => {
            this.tables_.push(new NotionTableWrapper(table, href, pageTitle, windowName, this.userConfig_));
          });
    }
    ...
    window.document.body
        ...
        .forEach(table => {
          const wrapper = new TableWrapper(table, href, windowName, this.userConfig_);
          if (wrapper.isValid()) {
