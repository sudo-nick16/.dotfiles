...
class ElementWrapper {
  constructor(element, url, userConfig) {
    ...
    this.url = url;
    this.pageTitle = typeof document !== "undefined" && document && document.title
        ? document.title
        : null;
  ...
  getRecipeDefinition() {
    return {
      ...
      selector: _tcSpecificGetSelectorForEl(this.dom.element),
      disabled: false,
      autoClip: false,
    };
  }
  ...
  getFilename() {
    return ExcelUtil.idToName(this.getTableId(), this.userConfig_.filenameTemplate);
  }
  ...
  performRepro(repro) {
    ...
    return Promise.reject(new Error("Unsupported Table Capture reproduction: " + repro.messageAction));
  }

  excel () {
    _tcLogTableWrapperMessageAction(this, MessageAction.EXCEL, OutputFormat.EXCEL, this.userConfig_);
  ...
  csv () {
    _tcLogTableWrapperMessageAction(this, MessageAction.CSV, OutputFormat.CSV, this.userConfig_);

    const data = this.getAsArrays();
    const filename = this.getFilename();
    return ExcelUtil.exportArrayOfArraysToCSVP(data, 'Sheet', filename, this.userConfig_.csvDelimiter);
  }
  ...
  scrollTo () {
    try {
      ...
      if (!_tcIsBehindOtherElement(this.dom.element, false)) {
        return;
      }
      ...
      this.scrollTimeout_ = window.setTimeout(() => {
        ...
        if (_tcIsBehindOtherElement(this.dom.element, true)) {
          this.dom.element.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
            inline: 'center',
      ...
      document.body.scrollTop = this.dom.element.offsetTop;
    }
  }

  select () {
    this.dom.element.classList.add(CLASSNAME_TABLE_SELECTED);
    this.scrollTo();
  }

  deselect () {
    this.dom.element.classList.remove(CLASSNAME_TABLE_SELECTED);
  }
  ...
  highlight () {
    ...
    this.dom.element.classList.add(CLASSNAME_TABLE_HIGHLIGHTED);
    this.scrollTo();
  }
  ...
  flash() {
    this.dom.element.classList.add(CLASSNAME_TABLE_HIGHLIGHTED);
    window.setTimeout(() => {
      removeClass(this.dom.element, CLASSNAME_TABLE_HIGHLIGHTED);
    }, 500);
  }

  screenshotDataUrl() {
    return ScreenshotUtil.html2canvas(this.dom.element);
  }

  screenshot () {
    return ScreenshotUtil
        .html2canvas(this.dom.element)
        .then(dataUrl => {
          if (this.userConfig_.copyImagesToClipboard) {
            Clipboard.copyImage(dataUrl);
          } else {
            const blob = new Blob([ScreenshotUtil.base64ToUint8Array(dataUrl)], {type: 'image/png'});
