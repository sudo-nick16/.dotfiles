...
class ClipManager {
  ...
  initialize(focusCollectionId=null) {
    this.fetchClips_()
        .then(collections => this.renderCollections_(collections, focusCollectionId))
  ...
  fetchClips_() {
    return chrome.extension.getBackgroundPage().getClipCollections();
  }
  ...
  renderCollections_(collections, focusCollectionId) {
    ...
    if (Object.keys(collections).length === 0) {
      wrapper.appendChild(createAlertPane('You have no clipped collections.', 'warning', false));
      return;
    }
    ...
    Object
        ...
        .forEach(collection => {
          ...
          el.querySelector('.preview-unique-btn')
              .addEventListener('click', () => {
                chrome.extension
                    .getBackgroundPage()
                    .saveTableLocally(this.createPublicTableFromCollection_(collection, dedupedData));
              });
        });
  }
  ...
  getUniqueDataset_(collection) {
    ...
    collection.data.forEach(entry => {
      ...
      if (datasetsByHeaders[headerHash]) {
        ...
        datasetsByHeaders[headerHash].data = [...datasetsByHeaders[headerHash].data, ...rest];
      } else {
    ...
    // Util function
    const nEmptyStrings = n => " ".repeat(n).split("").map(el => el.trim());

    Object
        .values(datasetsByHeaders)
        .forEach((dataset, index) => {
          // Add a separator between datasets.
          if (index !== 0) {
            const numEmptyCellsToAdd = dataset.data && dataset.data.length > 0
                ? dataset.data[0].length
                : 1;
